...
for i_pfa = 1:length(Pfas_fix)
    ...
    b = randn(nb_obs, nb_rea)*sqrt(var_b);
    s = randn(nb_obs, nb_rea)*sqrt(var_b) + m;
    ...
    seuil_lambda = sqrt(2*nb_obs*var_b)*erfcinv(2*Pfas_fix(i_pfa));
    ...
    Pd_th = 1/2*erfc( (seuil_lambda - nb_obs*m)/sqrt(2*nb_obs)*sqrt(var_b) );
    Pfa_th = 1/2*erfc(seuil_lambda/sqrt(2*nb_obs*var_b));
